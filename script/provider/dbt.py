...
from pvcli.run import Dataset, Job, OutputDataset, Run, RunEvent, RunState
...
class SkipUndefined(Undefined):
    ...
    def __call__(self, *args, **kwargs):
        arguments = ', '.join([arg._undefined_name if isinstance(arg, SkipUndefined) else str(arg) for arg in args])
...
class DbtArtifactProcessor:
    def __init__(
        ...
        print("project path: "+os.path.join(project_dir, 'dbt_project.yml'))
    ...
    def parse(self) -> DbtEvents:
        """
            Parse dbt manifest and run_result and produce Lineage events.
        """
    ...
    def extract_namespace(self, profile: Dict) -> str:
        ...
        if self.adapter_type == Adapter.POSTGRES:
            ...
            if profile['type'] in ['databricks']:
                return f"https://{profile['host']}"
            else:
                raise NotImplementedError(
                    f"Connection method `{profile['type']}` is not "
                    f"supported for databricks adapter."
                )
        else:
            raise NotImplementedError(
                f"Only {Adapter.adapters()} adapters are supported right now. "
